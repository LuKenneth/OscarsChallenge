...
class AsymmetricView extends StatelessWidget {
  ...
  AsymmetricView({Key key, this.nominees});
  ...
  List<Container> _buildColumns(BuildContext context) {
    ...
    return List.generate(_listItemCount(nominees.length), (int index) {
      ...
      if (index % 2 == 0) {
        ...
        column = TwoProductCardColumn(
            bottom: nominees[bottom],
            top: nominees.length - 1 >= bottom + 1
                ? nominees[bottom + 1]
                : null);
        width += 32.0;
      } else {
        ...
        column = OneProductCardColumn(
          product: nominees[_oddCasesIndex(index)],
        );
      }
      return Container(
        width: width,
